...
export function WorkflowsTableRow({
  ...
  async function handleRun() {
    setIsStarting(true);
    try {
      await runWorkflow({ workspaceId, workflowId: workflow._id });
  ...
  async function handleStop() {
    try {
      await stopRun({ workspaceId, workflowId: workflow._id });
    } catch {
    ...
    <WorkflowRow.Provider resource={workflow}>
      ...
      >
        <WorkflowRow.NameCell
          ...
          href={`/workspace/${workspaceId}/workflow/${workflow._id}/canvas`}
