...
export function RunWorkflowButton() {
  ...
  if (!canWrite) {
    runTitle = 'Viewers have read-only access to this workspace.';
  } else if (!isPublished) {
  ...
  if (runPhase === 'local') {
    return (
      <Button
        ...
        disabled={isStopping}
        onClick={() => {
          stopWorkflow({ workspaceId, workflowId });
        }}
    ...
    <Button
      ...
      onClick={async () => {
        ...
        if (isRerun && newRunId !== undefined) {
          router.push(
            `/workspace/${workspaceId}/workflow/${workflowId}/run-history/${newRunId}`
          );
        }
      }}
      className='w-36 gap-1.5'
