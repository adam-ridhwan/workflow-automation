    ...
    try {
      ...
      const report = (loadedTotal: number) => {
        ...
        if (now - lastSent >= PROGRESS_INTERVAL_MS || progress >= 100) {
          lastSent = now;
          setUploadProgress({ workspaceId, fileId, progress });
        }
      };
      ...
      if (file.size <= CHUNK_SIZE) {
        ...
        await finishUpload({ workspaceId, fileId, storageId });
        return;
      }
      ...
      for (let start = 0; start < file.size; start += CHUNK_SIZE) {
        ...
        await addChunk({ workspaceId, fileId, storageId });
        uploadedBytes += chunk.size;
        report(uploadedBytes);
      }
      await finishUpload({ workspaceId, fileId });
    } catch (err) {
      // Drop the stuck `uploading` row (and its chunk blobs).
      await cancelUpload({ workspaceId, fileId }).catch(() => {});
      throw err;
    }
  }
