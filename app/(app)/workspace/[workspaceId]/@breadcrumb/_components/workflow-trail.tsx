...
export async function WorkflowTrail({
  ...
  const folderPath = workflow.folderId
    ? await fetchQuery(
        api.folders.path,
        { workspaceId: workspaceId, folderId: workflow.folderId },
        { token }
      )
    : null;
  ...
  const trail = [
    ...(folderPath ?? []).map((segment) => ({
      ...
      name: segment.name,
      href: `/workspace/${workspaceId}/workflows/folder/${segment._id}`,
      icon: 'folder' as const,
    })),
    {
      id: workflow._id as string,
      name: workflow.name,
      href: `/workspace/${workspaceId}/workflow/${workflow._id}`,
